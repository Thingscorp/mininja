...
Quality / product metaphors only. **Not affiliated with** Bandai (Giga Pets / Tamagotchi-class toys), Pebble Technology, IFTTT Inc., or Apple Inc. Spell it **Mininja**. The mascot has **no personal name** (never Casque); no he/him.
...
|--------|----------------|----------------|
| **Giga Pets** | A creature you care about — warmth via faces / presence you *feel* | Digipet hunger/feeding loops, Farmville dashboards, gacha chrome, naming the mascot |
...
Together they describe **one connected system**. **v1 hero** = creature + habitat glass. Recipes stay **later plate** on the same graph — demotion ≠ disconnection. Do **not** read the three parents as ship-first “creature + face + recipes.”
...
3. **Warmth without clutter** — Giga Pets–class creature warmth (faces / presence) without digipet care loops or another status dashboard. Garden growth is **repo** silhouette vocab in habitat (`repoBranch`), not pet leveling.
...
If a decision fights this north star (bloat the face, dual-table emotions, hero the recipe runner before the terrarium, equate faces with emotions), stop and re-read the three parents.
