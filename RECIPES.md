...
Recipes are **later programs** on the language’s structural ids — events → expressions that address the same kit addresses, so a future runner plugs into creature + habitat without a second lexicon. See [LANGUAGE.md](LANGUAGE.md).
...
They share **one vocabulary**. Changing a face id, stage id, or growth field must stay **recipe-compatible** so a later program can still target it. v1 ships creature + habitat (+ optional garden props) **without** a recipe runner; the seams stay open.
