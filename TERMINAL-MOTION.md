...
Formal locomotion laws for the Mininja mark in terminal / console surfaces — the language’s **verbs** ([LANGUAGE.md](LANGUAGE.md)). Machine source of truth: [`kit/scene.json`](kit/scene.json). Console `src/lib/scene.ts` / `banner.tsx` are a historical source and must stay aligned to kit — they are not live SoT. The mascot has no name.
