...
| **P1** | **Multi-pal color in habitat glass** — concurrent pals, color-distinguished, one-pal↔one-plant visualization | Tint exists for **selected** bot chrome only; React banner is single actor; grove ≠ garden plants | **Apps** host layer (tint algorithm may stay). **MUST NOT** kit pal-color table ([`GLANCE.md`](../GLANCE.md)). |
| **P1** | **Pal ↔ `repoBranch` plant binding** in UI | Design in GLANCE/GARDEN; kit has growth bricks; hosts don’t yet show multi-pal on plants | **Apps** visualization. **Kit** already owns `repoBranch` / growth ids — do not invent new. |
